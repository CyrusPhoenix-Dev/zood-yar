...
import api from "../api";
import "../styles/EditPhone.css";

const RESEND_SECONDS = 60;

// Swap with real data from api.get("/api/user/profile/") — static for now.
const currentPhone = "09121234567";

function EditPhonePage() {
  const [step, setStep] = useState("current"); // "current" | "new" | "code" | "done"
  ...
  const handleSendCode = async (e) => {
    ...
    setLoading(true);
    try {
      // TODO: confirm endpoint — separate from the public login-OTP
      // endpoint, since this is an authenticated action tied to the
      // current user, not a login flow. `api` already attaches the
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setError("ارسال مجدد کد ناموفق بود");
      console.error(err);
    } finally {
      setLoading(false);
    }
